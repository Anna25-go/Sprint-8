...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("Error adding row: %w", err)
	}
...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return Parcel{}, fmt.Errorf("Parcel with id %d not found", number)
		}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	err = rows.Err()
	if err != nil {
		return nil, fmt.Errorf("Error iterating over parcels for client %d: %w", client, err)
	}
